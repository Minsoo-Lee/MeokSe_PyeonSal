...
2. **Networks → Tunnels** → 지금 쓰고 있는 터널(`chatshire-home-server`) 클릭
3. **"Published application routes"** 탭 → **Add a published application route**를 두 번:

   | Domain | Path | Service |
   |---|---|---|
   | ochangbab.site | (비워둠 — `*` 직접 입력하지 말 것, 정규식으로 해석돼서 에러남) | `http://localhost:3000` |
   | api.ochangbab.site | (비워둠) | `http://localhost:8082` |

저장하면 DNS 레코드도 자동 생성되고, cloudflared 재시작도 필요 없이 바로 반영됨 (원격 관리형이라).

(참고: "Public Hostname"이나 "Hostname routes"가 아니라 **"Published application routes"** 탭이 맞는 곳임 — Cloudflare UI 이름이 바뀌어서 헷갈리기 쉬움.)
